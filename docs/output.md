...
- [dragen](#dragen) - converting bcl files to fastq, and demultiplexing by Illumina Dragen (CONDITIONAL)
...
- [interop](#interop) - Illumina InterOp metrics
- [FastqScreen](#fastqscreen) - Screen fastq files against a set of reference genomes
- [Kraken2](#kraken2) - Taxonomic classification of fastq files
- [Krona](#krona) - Generate Krona plots from Kraken2 output
...
- `<flowcell_id>/`
   - `*.fastq.gz`: Untrimmed raw fastq files
...
[bcl2fastq](https://support.illumina.com/sequencing/sequencing_software/bcl2fastq-conversion-software.html) Conversion Software both demultiplexes data and converts BCL files generated by Illumina sequencing systems to standard FASTQ file formats for downstream analysis.
...
[dragen](https://www.illumina.com/products/by-type/informatics-products/edico-genome.html) BCL Convert is a standalone local software app that converts the Binary Base Call (BCL) files produced by Illumina™ sequencing systems to FASTQ files.
...
- `<flowcell_id>/falco/`
  - `*_fastqc_report.html`: FastQC report containing quality metrics.
  - `*_fastqc_data.txt`: Txt containing the FastQC report, tab-delimited data file.
...
[interop](https://illumina.github.io/interop/index.html) Conversion Software both demultiplexes data and converts BCL files generated by Illumina sequencing systems to standard FASTQ file formats for downstream analysis.

<details markdown="1">
<summary>Output files</summary>

- `<flowcell_id>/InterOp`
  - `interop_index_summary_report.csv`: InterOp index summary report.

### FastqScreen

[FastqScreen](https://www.bioinformatics.babraham.ac.uk/projects/fastq_screen/) allows you to screen a library of sequences in FastQ format against a set of sequence databases so you can see if the composition of the library matches with what you expect.

<details markdown="1">
<summary>Output files</summary>

- `<flowcell_id>/fastq_screen/`
  - `*screen.html`: FastqScreen HTML report
  - `*screen.txt`:  FastqScreen text report
  - `*screen.png`:  FastqScreen PNG plot

### Kraken2

[kraken2](https://ccb.jhu.edu/software/kraken2/) is a taxonomic sequence classifier that assigns taxonomic labels to short DNA sequences.

<details markdown="1">
<summary>Output files</summary>

- `<flowcell_id>/kraken2/`
  - `*.classified{.,_}*`: classified reads fastq.
  - `*.unclassified{.,_}*`:: unclassified reads fastq.
  - `'*classifiedreads.txt`: classified reads assignment.
  - `*report.txt`: kraken report (kreport).

- `<flowcell_id>/krakentools/`
  - `*.txt`:   kreport to krona text report.


### Krona

[krona](https://github.com/marbl/Krona/wiki) is a hierarchical data visualization tool that can be used to visualize taxonomic classifications.

<details markdown="1">
<summary>Output files</summary>

- `<flowcell_id>/krona/`
  - `*.html`:  krona html report.
